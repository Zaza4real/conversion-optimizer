...
let RootController = RootController_1 = class RootController {
    constructor(shops, config, billing) {
        this.shops = shops;
        this.config = config;
    ...
    landing(req, res) {
        ...
        const appStoreUrl = this.config.get('APP_STORE_LISTING_URL') || '#';
    ...
    billingConfirm(req, res) {
        const shop = req.query.shop?.trim();
        const requestedPlan = req.query.plan?.toLowerCase().trim();
        ...
        const isSameCurrentPlan = async () => {
            const existing = await this.shops.findByDomain(normalized);
            if (!existing || !this.shops.hasPaidPlan(existing))
                return false;
    ...
    async index(req, res) {
        ...
        if (forceReauth) {
            ...
            res.setHeader('Content-Type', 'text/html; charset=utf-8');
            res.send(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Conversion Optimizer</title></head><body><p>Loading Conversion Optimizer…</p><script>window.top.location.href=${JSON.stringify(authUrl)};</script></body></html>`);
            return;
        }
        ...
        try {
            ...
            if (msg.includes('SHOP_RECONNECT_REQUIRED')) {
                ...
                res.setHeader('Content-Type', 'text/html; charset=utf-8');
                res.send(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Conversion Optimizer</title></head><body><p>Refreshing authorization…</p><script>window.top.location.href=${JSON.stringify(reconnectUrl)};</script></body></html>`);
                return;
            }
        ...
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate');
        const appStoreListingUrl = this.config.get('APP_STORE_LISTING_URL');
    ...
    getAppBridgeHead() {
        const apiKey = this.config.get('SHOPIFY_API_KEY')?.trim();
        if (!apiKey)
            return '';
    ...
    getAppHomeHtml(shop, hasPlan, currentPlanLabel, currentPlanKey, baseUrl, billingError = false, appStoreListingUrl, billingSuccess = false, planJustPurchased = '', cancelled = false, billingCancelError = false, isFreeBeta = false, samePlan = false, activeUntilIso = '', cancelledPlanLabel = '') {
        ...
        const billingCard = hasAccess
            ...
            : '';
        const plansCard = `<div class="card"><p class="card-title">Plans</p><p class="card-text">${hasPlan ? 'Change plan or manage billing below. ' : ''}Cancel anytime from the app or your Shopify billing.</p><div class="plans-grid"><!-- BILLING_BADGE_FIX_ENABLED_v4 -->${plansDisplay.map((p) => {
...
<body>
  ...
  ${this.getDismissAppBridgeLoadingScript()}
    ...
    getBillingConfirmHtml(planKey, baseUrl, homeUrl, subscribeUrl) {
        const planName = planKey === 'pro_annual' ? 'Pro Annual' : planKey === 'pro' ? 'Pro' : planKey === 'starter' ? 'Starter' : 'Growth';
...
<body>
  ...
  ${this.getDismissAppBridgeLoadingScript()}
    ...
    getBillingCancelConfirmHtml(baseUrl, homeUrl, cancelUrl) {
        return `<!DOCTYPE html>
<html lang="en">
<head>
  ${this.getAppBridgeHead()}
...
<body>
  ...
  ${this.getDismissAppBridgeLoadingScript()}
    ...
    getScanRunPageHtml(shop, apiUrl, homeUrl, recsUrl) {
...
<body>
  ...
  ${this.getDismissAppBridgeLoadingScript()}
    ...
    getRecommendationsPageHtml(shop, apiUrl, homeUrl) {
        return `<!DOCTYPE html>
<html lang="en">
<head>
  ${this.getAppBridgeHead()}
...
<body>
  ...
  ${this.getDismissAppBridgeLoadingScript()}
    ...
    getBackUrl() {
        const appStore = this.config.get('APP_STORE_LISTING_URL')?.trim();
        if (appStore && appStore !== '#')
            return appStore;
        return 'https://apps.shopify.com/conversion-optimizer';
    }
    getBackUrlFromRequest(req) {
        const raw = req.query?.return_to?.trim();
        if (raw && raw.startsWith('https://')) {
            try {
                const u = new URL(raw);
                if (u.protocol === 'https:' && u.hostname.endsWith('.myshopify.com'))
                    return raw;
            }
            catch {
            }
        }
        const defaultBack = this.config.get('DEFAULT_BACK_URL')?.trim();
        if (defaultBack && defaultBack.startsWith('https://')) {
            try {
                const u = new URL(defaultBack);
                if (u.protocol === 'https:' && u.hostname.endsWith('.myshopify.com'))
                    return defaultBack.replace(/\/$/, '') + '/';
            }
            catch {
            }
        }
        return RootController_1.STORE_BACK_URL;
    }
    getPolicyHtml(title, bodyHtml, backUrl) {
...
<p>For billing or refund questions, use the support contact provided in the app listing.</p>`;
    ...
    getSupportPageHtml(backUrl) {
        const baseUrl = this.config.get('SHOPIFY_APP_URL')?.replace(/\/$/, '') ?? '';
        const url = backUrl ?? this.getBackUrl();
        const supportEmail = this.config.get('SUPPORT_EMAIL')?.trim() || '';
...
<body>
  <div class="wrap">
    ...
    getLandingPageHtml(baseUrl, appStoreUrl) {
...
RootController.STORE_BACK_URL = 'https://conversionoptimizer.myshopify.com/';
__decorate([
...
], RootController.prototype, "favicon", null);
__decorate([
...
__decorate([
    ...
    __metadata("design:returntype", void 0)
], RootController.prototype, "privacy", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", void 0)
], RootController.prototype, "refund", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", void 0)
], RootController.prototype, "support", null);
__decorate([
    (0, common_1.Get)('health/billing-repair'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], RootController.prototype, "billingRepair", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", void 0)
], RootController.prototype, "landing", null);
__decorate([
    (0, common_1.Get)('scan/run'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", void 0)
], RootController.prototype, "scanRunPage", null);
__decorate([
    (0, common_1.Get)('billing/cancel-confirm'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", void 0)
], RootController.prototype, "billingConfirm", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", void 0)
], RootController.prototype, "recommendationsPage", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Res)()),
